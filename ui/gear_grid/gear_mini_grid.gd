extends Control
## 게임 플레이 중 우측 하단에 표시되는 기어 미니 그리드
...
func _ready() -> void:
	_build_grid()
	MetaState.gear_grid.grid_changed.connect(_refresh_colors)
...
## 효과가 발동한 기어들의 셀에 펄스 애니메이션 재생
func highlight_gears(gears: Array[GearInstance]) -> void:
	_kill_active_tweens()

	for gear in gears:
		if not gear.is_placed:
			continue
		for cell_pos in gear.get_occupied_cells():
			if cell_pos.x < 0 or cell_pos.x >= GRID_SIZE or cell_pos.y < 0 or cell_pos.y >= GRID_SIZE:
				continue
			var idx := cell_pos.y * GRID_SIZE + cell_pos.x
			_animate_cell_pulse(_grid_cells[idx], gear.type.color)
